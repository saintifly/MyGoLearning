...
function S = L0main5(Im,Ip, lambda, kappa)  
...
%     
    ...
    %     Denormin(:,:,1)   = 1 + sum((h.^2+v.^2),3).*beta.*Denormin2(:,:,1);
%      Denormin(:,:,2)   = 1 + sum((h.^2+v.^2),3).*beta.*Denormin2(:,:,2);
%       Denormin(:,:,3)   = 1 + sum((h.^2+v.^2),3).*beta.*Denormin2(:,:,3);
%       FS1=FS;
%     FS1(:,:,1) = (Normin11(:,:,1) + sum((h.^2+v.^2),3).*beta.*fft2( Normin21(:,:,1)))./Denormin(:,:,1);
%      FS1(:,:,2) = (Normin11(:,:,1) + sum((h.^2+v.^2),3).*beta.*fft2( Normin21(:,:,2)))./Denormin(:,:,2);
%       FS1(:,:,3) = (Normin11(:,:,1) + sum((h.^2+v.^2),3).*beta.*fft2(
%       Normin21(:,:,3)))./Denormin(:,:,3);
