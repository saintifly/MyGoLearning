function sum=hboxfilter(patch_darkchanel,r)
   
[m,n]=size(patch_darkchanel);

sum_h= cumsum(patch_darkchanel,2);

sum=zeros(m,n);

sum(:,1:r+1) = sum_h(:,r+1:2*r+1);

sum(:,r+2:n-r) = sum_h(:,2*r+2:n)-sum_h(:,2:n-2*r);

sum(:,n-r+1:n) = sum_h(:,n)*ones(1,r)-sum_h(:,n-2*r+1:n-r);
